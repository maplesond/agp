    ...
    for line in gff:
        ...
        if parts[0] in problems:
            ...
            if gff_start >= start and gff_start <= end or gff_end >= start and gff_end <= end:
                print("Problem in scaffold: " + p.scaf, file=sys.stderr)

            if (p.start):
                mod_parts[3] = str(gff_start - end)
                mod_parts[4] = str(gff_end - end)
                print("Modified " + parts[0], file=sys.stderr)

        print("\t".join(mod_parts), end="")
